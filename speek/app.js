...
async function sendMessage() {
    ...
    aiBox.innerHTML = `
        ...
        <div class="msg-content">
            ...
            <div class="model-tag" style="display:none;"></div>
        </div>
    `;
    chatbox.appendChild(aiBox);
    scrollBottom();
    const aiBubble = aiBox.querySelector(".bubble");
    const aiModelTag = aiBox.querySelector(".model-tag");
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (obj.type === "model") {
                        ...
                        if (aiModelTag && obj.label) {
                            const isArk = obj.backend === "ark";
                            aiModelTag.textContent = (isArk ? "☁️ " : "🤖 ") + obj.label + (obj.fell_back ? "（本地失败·兜底）" : "");
                            aiModelTag.style.display = "block";
                        }
                        continue;
                    }
                    if (obj.error) {
